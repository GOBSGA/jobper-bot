...
import { List, X, Bell, SignOut } from "@phosphor-icons/react";
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl border border-surface-border shadow-md z-50 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border">
        <p className="text-xs font-semibold text-ink-900">Notificaciones</p>
        <button
          onClick={onClose}
          className="text-ink-400 hover:text-ink-600 transition-colors p-0.5 rounded"
        >
          <X size={15} />
        </button>
      </div>
      <div className="max-h-72 overflow-y-auto divide-y divide-surface-hover">
        {loading && (
          <p className="text-xs text-ink-400 text-center py-6">Cargando...</p>
        )}
        {!loading && contracts.length === 0 && (
          <p className="text-xs text-ink-400 text-center py-6">
            Sin contratos nuevos en las últimas 48 h.
          </p>
        )}
        {contracts.map((c) => (
          <a
          ...
          >
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              <span className="text-2xs font-semibold text-brand-600">{money(c.amount)}</span>
            </div>
            {c.deadline && (
      ...
      {contracts.length > 0 && (
        <div className="px-4 py-3 border-t border-surface-border">
          <a href="/contracts" className="text-xs text-brand-500 hover:text-brand-700 font-medium">
...
export default function Navbar() {
  ...
  return (
    <header className="lg:hidden sticky top-0 z-30 bg-white border-b border-surface-border">
      ...
      {open && (
        <nav className="border-t border-surface-border px-3 py-2 space-y-px">
          {[
            ["/dashboard",   "Dashboard"],
            ["/contracts",   "Contratos"],
            ["/favorites",   "Favoritos"],
